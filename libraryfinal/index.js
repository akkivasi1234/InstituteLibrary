...
app.post("/object", urlencodedparser, (req, res) => {
    async function main() {
        try {
            ...
            const parent = network.getContract('object', 'Parent');
...
app.post("/rule", urlencodedparser, (req, res) => {
    function getDataFromServer2(data) {
        ...
        return fetch('http://35.224.182.132:8000/rule', {
            method: 'post',
            body: jsondata,
    ...
    async function main() {
        try {
            ...
            const sub = network.getContract('object', 'Sub');
...
app.post("/access", urlencodedparser, (req, res) => {
    function getDataFromServer2(data) {
        const jsondata = JSON.stringify(data);
        return fetch('http://35.224.182.132:8000/access', {
            method: 'post',
            body: jsondata,
            headers: { 'Content-Type': 'application/json' },
        }).then(res => res.json())
    ...
    async function main() {
        try {
            ...
            var results = [];
            const sub = network.getContract('object', 'Sub');
            let result2 = await sub.evaluateTransaction('accessrequest', result[objectOrganization][identifier], JSON.stringify(attributesToBeVerifiedByOrganization));
