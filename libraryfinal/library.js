...
class Parent extends Contract {
    ...
    async divideIntoSubRules(ctx, data, attributesToBeVerifiedByOrganizations, namesOfOrganizations) {
        ...
        attributesToBeVerifiedByOrganizations = JSON.parse(attributesToBeVerifiedByOrganizations);
        namesOfOrganizations = JSON.parse(namesOfOrganizations);
        var result = {};
        for (let i = 1; i <= namesOfOrganizations.length; i++) {
            var organization = namesOfOrganizations[i - 1];
            var attributesToBeVerifiedByOrganization = attributesToBeVerifiedByOrganizations[organization];
    ...
    async divideIntoSubRequests(ctx, data, organizationsIdentifiers, namesOfOrganizations) {
        ...
        organizationsIdentifiers = JSON.parse(organizationsIdentifiers);
        namesOfOrganizations = JSON.parse(namesOfOrganizations);
        const result = {};
        for (let i = 1; i <= namesOfOrganizations.length; i++) {
            var organization = namesOfOrganizations[i - 1];
...
class Sub extends Contract {
    ...
    async accessrequest(ctx, objectIdentifier, attributesToBeVerifiedByOrganization) {
        const result = [];
        const fileAsBytes = await ctx.stub.getState(objectIdentifier);
        ...
        while (true) {
            ...
            for (j = 1; j <= attributesToBeVerifiedByOrganization.length; j++) {
                ...
                if (((attribute != "Sensitivity") && (file[attribute]) != rule[attribute]) || ((attribute == "Sensitivity") && (file[attribute] > rule[attribute]))) {
                    result.push(false);
                    break;
                }
            }
